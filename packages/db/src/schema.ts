...
export const subscriptions = sqliteTable(
  "subscriptions",
  {
    ...
    /**
     * Metered subscription-item ID for Scale-tier usage reporting.
     * Set by the Stripe webhook when a customer subscribes via the metered
     * SKU (price tier configured in Stripe dashboard).
     */
...
export const apiKeys = sqliteTable(
  "api_keys",
  {
    ...
    organizationId: text("organization_id")
      .notNull()
      .references(() => organizations.id, { onDelete: "cascade" }),
    // The user who minted the key. Nullable for keys created before this
    // column existed; in that case the auth middleware falls back to the
    // first-member-of-org behaviour. New keys must populate this.
...
export const links = sqliteTable(
  "links",
  {
    ...
    expiresAt: text("expires_at"),
    // Policy-driven expiry (separate from user-set expiresAt). Free tier gets
    // 60 days from createdAt; paid tiers leave this null. Plan changes update
    // this column without touching the user-set `expiresAt`.
    ...
    // Agent attribution — populated when an AI agent (Claude Code, Cursor, etc.)
    // creates this link. Carried through to every click recorded against it.
    agentId: text("agent_id"),
    agentRunId: text("agent_run_id"),
    agentActorId: text("agent_actor_id"),
    agentMetadata: text("agent_metadata"), // JSON, free-form
...
export const domains = sqliteTable(
  "domains",
  {
    ...
    // Optional binding to a bio gallery: when set, requests to this hostname
    // resolve to that gallery's bio page. NULL means the domain is used for
    // link-shortener traffic only.
    bioGalleryId: text("bio_gallery_id"),
    // Timestamps
    createdAt: text("created_at")
      .notNull()
      .default(sql`(datetime('now'))`),
...
export const clicks = sqliteTable(
  "clicks",
  {
    ...
    // Agent attribution — populated from the link's stored agent context
    // and/or query/header overrides at click time.
    agentId: text("agent_id"),
    agentRunId: text("agent_run_id"),
    agentActorId: text("agent_actor_id"),
...
// -----------------------------------------------------------------------------
//
// First-class record of an AI agent run. Before this table existed, "runs"
// were a derived view over clicks — runs that created links but received no
// clicks were invisible. Now: every link create with an `agent_run_id`
// auto-upserts a row here, so the dashboard / MCP `list_agent_runs` can show
// pending runs immediately.
...
export const agentRuns = sqliteTable(
  "agent_runs",
  {
    id: text("id").primaryKey(), // UUID — internal row id
    // Workspace scope. Both populated for org-bound API keys; userId may be
    // populated alone for personal-tier keys.
    userId: text("user_id")
      .notNull()
      .references(() => users.id, { onDelete: "cascade" }),
    organizationId: text("organization_id").references(() => organizations.id, {
    ...
    // Caller-provided agent context. (agentId, runId) is unique within a
    // workspace — the same agent can't double-register the same run.
    agentId: text("agent_id").notNull(),
    runId: text("run_id").notNull(),
    actorId: text("actor_id"),
    // Lifecycle status. Auto-created runs start as "running" and stay that
    // way until an explicit end_run / revoke. The dashboard should show all
    // statuses but allow filtering.
    status: text("status", { enum: agentRunStatuses })
      .notNull()
      .default("running"),
    // Denormalized counts — kept up to date by triggers / app code so the
    // run-list dashboard doesn't have to JOIN clicks every time.
    linkCount: integer("link_count").notNull().default(0),
    clickCount: integer("click_count").notNull().default(0),
    metadata: text("metadata"), // JSON, capped 4 KB at the API boundary
  ...
  (table) => [
    uniqueIndex("agent_runs_workspace_run_idx").on(
      table.userId,
      table.agentId,
      table.runId,
    ),
    index("agent_runs_org_idx").on(table.organizationId),
    index("agent_runs_status_idx").on(table.status),
    index("agent_runs_started_idx").on(table.startedAt),
    index("agent_runs_actor_idx").on(table.actorId),
  ],
);
...
export const folders = sqliteTable(
  "folders",
  {
    ...
    // Stats
    linkCount: integer("link_count").notNull().default(0),
    // Soft delete: timestamp the folder was archived. NULL = live folder.
...
export const linkGalleries = sqliteTable(
  "link_galleries",
  {
    ...
    // Hide the "Powered by Go2" footer on the public page (Pro+ feature).
    hideBranding: integer("hide_branding", { mode: "boolean" }).notNull().default(false),
    // Flagged as the primary gallery for its (organization, domain) pair —
    // the one served at "/" when a custom domain is bound. NULL/0 means the
    // gallery is only reachable at /bio/<slug>.
...
// Gallery items (links, headers, embeds, email_signup)
...
export const galleryItems = sqliteTable(
  "gallery_items",
  {
    ...
    clickCount: integer("click_count").notNull().default(0),
    lastClickedAt: text("last_clicked_at"),
    // For embeds + email_signup config + gate config (kind: 'password'|'email', value: hashed)
    embedType: text("embed_type"), // youtube, tiktok, spotify, etc.
    embedData: text("embed_data"), // JSON
    // OG-unfurl cache (server-fetched at link save):
    ogTitle: text("og_title"),
    ogDescription: text("og_description"),
...
// Bio email signup subscribers (collected from `email_signup` items + email
// gates on gated content). One row per (gallery, email).
export const bioSubscribers = sqliteTable(
  "bio_subscribers",
  {
    id: text("id").primaryKey(), // UUID
    galleryId: text("gallery_id")
      .notNull()
      .references(() => linkGalleries.id, { onDelete: "cascade" }),
    itemId: text("item_id").references(() => galleryItems.id, {
      onDelete: "set null",
    }),
    email: text("email").notNull(),
    confirmed: integer("confirmed", { mode: "boolean" }).notNull().default(false),
    confirmToken: text("confirm_token"),
    // 'signup' = email_signup item; 'gate' = email-gated link unlock
  ...
  (table) => [
    index("bio_subs_gallery_idx").on(table.galleryId),
    uniqueIndex("bio_subs_unique_idx").on(table.galleryId, table.email),
  ],
);

export type BioSubscriber = typeof bioSubscribers.$inferSelect;
export type NewBioSubscriber = typeof bioSubscribers.$inferInsert;
...
export const conversions = sqliteTable(
  ...
  (table) => [
    ...
    // Race-safe dedup: see drizzle/0013_conversion_dedup.sql. The partial
    // unique index uses COALESCE(goal_id, '') so NULL goals also dedup.
    // Drizzle Kit can't represent the COALESCE expression yet, so the index
    // is hand-written in the migration.
  ],
);
...
// =============================================================================
// OAuth 2.1 (used by remote MCP and third-party agent integrations)
// =============================================================================
...
export const oauthAuthorizationCodes = sqliteTable(
  ...
  (table) => [
    index("oauth_authz_codes_client_idx").on(table.clientId),
    index("oauth_authz_codes_user_idx").on(table.userId),
    index("oauth_authz_codes_expires_idx").on(table.expiresAt),
  ],
);
...
// =============================================================================
// Email Suppressions (bounce / complaint / manual block)
// =============================================================================

/**
 * One row per suppressed recipient. `sendEmail` checks this table before
 * dispatching to the CF Email binding, and upserts a row when the binding
 * throws a hard-bounce-class error. Manual entries (test accounts, abuse
 * reports) use reason='manual'.
 */
export const emailSuppressions = sqliteTable(
  "email_suppressions",
  {
    email: text("email").primaryKey(),
    reason: text("reason").notNull(), // "hard_bounce" | "complaint" | "manual" | "invalid"
    source: text("source"), // template name or "manual:<note>"
  ...
  (table) => [index("email_suppressions_reason_idx").on(table.reason)],
);
