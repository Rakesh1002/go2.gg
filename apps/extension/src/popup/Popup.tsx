...
export function Popup() {
  ...
  // Load folders once authenticated. Fails silently — folder picker is
  // optional and shouldn't block link shortening if the API call 403s for a
  // free-plan user.
  useEffect(() => {
    if (!user) return;
    chrome.storage.local.get(["token"], async (result) => {
      try {
        ...
        if (result.token) headers["Authorization"] = `Bearer ${result.token}`;
        const response = await fetch(`${API_URL}/api/v1/folders?perPage=100`, {
          headers,
        });
        if (!response.ok) return;
        const data = await response.json();
        setFolders(((data?.data ?? []) as FolderOption[]).slice(0, 50));
      } catch (err) {
        // Network error or 403 — leave folder picker empty.
  ...
  }, [currentUrl, recentLinks, selectedFolderId]);
