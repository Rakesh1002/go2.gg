 ...
 *
 * Argument order matches the rest of this module's helpers (message before
 * status) and the in-tree call sites. Pass an http status code (number) third.
 */
export function error(c: Context, message: string, status: number, code = "ERROR") {
  return c.json<ApiResponse>(
    {
      success: false,
      error: { code, message },
