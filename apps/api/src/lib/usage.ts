...
export async function getOrgUsage(
  ...
  // Query tracked clicks this month — count rows from the clicks table whose
  // timestamp falls in the current month. Earlier the implementation summed
  // all-time clickCount on links with lastClickedAt this month, which both
  // over-counted (old clicks rolled in) and under-counted (links with no
  // click this month dropped out entirely).
    ...
    ? await db
        ...
        .where(
          and(
            ...
            gte(schema.clicks.timestamp, monthStart)
          )
        )
    : await db
        .select({
