...
export interface ClickMetadata {
  ...
  // Agent attribution (populated from link defaults and/or click-time overrides)
  agentId: string | null;
...
export async function collectClickMetadata(
  ...
  // Resolve agent context: query/header overrides win over link defaults
  const agent = extractAgentContext(request, link);
  ...
  return {
    ...
    agentActorId: agent.agentActorId,
    agentToolCallId: agent.agentToolCallId,
...
/**
 * Short query-param keys we accept on the redirect URL. Kept short so they
 * don't visually pollute short URLs that get pasted into chats:
 *   ag = agent_id        ar = agent_run_id
 *   at = agent_tool_call_id
 *   au = agent_actor_id (the human user behind the agent)
 *
 * Server-to-server callers should prefer the x-agent-* headers instead, so the
 * params don't leak into referrer logs on the destination site.
 */
export const AGENT_QUERY_KEYS = ["ag", "ar", "at", "au"] as const;
...
export function extractAgentContext(request: Request, link: CachedLink): AgentContext {
  const url = new URL(request.url);
  const headers = request.headers;
  const cap = (s: string | null | undefined) => (s ? s.slice(0, 200) : null);

  return {
    agentId:
      cap(url.searchParams.get("ag")) ??
      cap(headers.get("x-agent-id")) ??
      cap(link.agentId) ??
      null,
    agentRunId:
      cap(url.searchParams.get("ar")) ??
      cap(headers.get("x-agent-run-id")) ??
      cap(link.agentRunId) ??
      null,
    agentToolCallId:
      cap(url.searchParams.get("at")) ?? cap(headers.get("x-agent-tool-call-id")) ?? null,
    agentActorId:
      cap(url.searchParams.get("au")) ??
      cap(headers.get("x-agent-actor-id")) ??
      cap(link.agentActorId) ??
      null,
  };
}

/**
 * Strip our agent query keys from a URL so they don't leak to the destination
 * site's referrer log or appear in the user's address bar after redirect.
 * Returns the URL string unchanged if there were no agent keys to strip.
 */
export function stripAgentQueryKeys(url: string): string {
  try {
    const u = new URL(url);
    let mutated = false;
...
export async function trackClickEnhanced(
  ...
  tracker.writeDataPoint({
    blobs: [
      ...
      metadata.agentId ?? "", // 14: agent ID
      metadata.agentRunId ?? "", // 15: agent run ID
    ...
    doubles: [
      ...
      metadata.longitude ? Number.parseFloat(metadata.longitude) : 0, // 1: longitude
      metadata.latitude ? Number.parseFloat(metadata.latitude) : 0, // 2: latitude
