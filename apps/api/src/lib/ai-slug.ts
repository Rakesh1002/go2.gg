 ...
 *
 * Routes through aigateway.sh (Moonshot Kimi K2.6 by default) when
 * AI_GATEWAY_API_KEY is configured, with Workers AI as fallback. The shape
 * the rest of the API sees is `(env, ...)` — callers pass `c.env`.
 */

import { sanitizeSlug, isReservedSlug, generateSlug } from "./slug.js";
import { complete as aigatewayComplete, AIGatewayError } from "./aigateway.js";

export type AICapableEnv = {
  AI: Ai;
...
/**
 * Single LLM call for short-prompt shortener tasks. Tries aigateway.sh first
 * when configured (cheap, cached, OpenAI-compatible) and falls back to
 * Workers AI llama-3.1-8b on any gateway error so the feature stays alive
 * if the gateway is down or the secret is missing.
 */
async function runShortenerLLM(
  env: AICapableEnv,
  prompt: string,
  ...
  if (env.AI_GATEWAY_API_KEY) {
    try {
      ...
      const detail = error instanceof AIGatewayError ? `${error.status ?? "?"}` : "unknown";
      console.warn(`[ai-slug] aigateway failed (${detail}), falling back to Workers AI`);
    }
  }
...
Return ONLY the slugs, one per line, no numbering or explanations.`;
  ...
  try {
    const text = await runShortenerLLM(env, prompt, { maxTokens: 200, temperature: 0.7 });
...
export async function generateSingleAISlug(
  ...
  const suggestions = await generateAISlugs(env, destinationUrl, {
    count: 1,
    maxLength,
    style: "memorable",
  });
 ...
 */
export async function generateAILinkPreview(env: AICapableEnv, destinationUrl: string): Promise<LinkPreview> {
...
DESCRIPTION: Your description here`;
  ...
  try {
    const text = await runShortenerLLM(env, prompt, { maxTokens: 200, temperature: 0.6 });
...
Provide a 1-2 sentence summary of when to post for maximum engagement. Be specific about days and times.`;
  ...
  try {
    const summary =
      (await runShortenerLLM(env, prompt, { maxTokens: 100, temperature: 0.5 })).trim() ||
...
export function analyzeClickPatterns(
  ...
  const sortedDays = Object.entries(dayCount)
    ...
    .map(([day]) => {
      ...
      return days[Number.parseInt(day)];
    });
...
Format: One suggestion per line, be concise (max 100 chars each).`;
  ...
  try {
    const text = await runShortenerLLM(env, prompt, { maxTokens: 300, temperature: 0.5 });
