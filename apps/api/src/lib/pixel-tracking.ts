...
src="https://ct.pinterest.com/v3/?tid=${pixel.pixelId}&noscript=1" /></noscript>`;
    ...
    case "custom":
      // Custom JS is intentional power-user surface, but we still need to
      // prevent the script from breaking out of its <script> block. Replace
      // the only two escape sequences that matter inside a <script>:
      //   </script  -> closes the block
      //   <!--      -> opens an HTML comment that swallows the rest
      // Both are safe to substitute with their backslash-escaped forms; the
      // browser's JS parser treats them identically.
      if (pixel.customScript) {
        const safe = pixel.customScript
