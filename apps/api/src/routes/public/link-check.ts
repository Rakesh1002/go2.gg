...
linkCheck.post("/", zValidator("json", checkSchema), async (c) => {
  ...
  const requestCount = rateLimitResult ? Number.parseInt(rateLimitResult, 10) : 0;
...
// biome-ignore lint/correctness/noUnusedVariables: kept for future Google Safe Browsing wiring
async function _checkGoogleSafeBrowsing(apiKey: string, url: string): Promise<SafeBrowsingResult> {
