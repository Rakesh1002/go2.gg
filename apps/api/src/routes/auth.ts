...
auth.on(["GET", "POST"], "/*", async (c, next) => {
  ...
  // Custom diagnostic / extension endpoints — defer to handlers registered
  // below this catch-all. Better Auth has no routes at these paths and would
  // otherwise 404 them.
  if (
    path.endsWith("/oauth-config") ||
    path.endsWith("/me") ||
    path.endsWith("/verify-turnstile")
  ) {
    return next();
  }
  ...
  try {
    ...
    if (response.status === 302) {
      ...
      // If Better Auth redirected to the configured errorURL, surface the
      // exact failure mode so we can debug it (state mismatch, invalid_grant,
      // misconfigured redirect URI, etc.).
      if (location?.includes("error=")) {
        try {
          const locUrl = new URL(location);
          console.error(`[AUTH DEBUG] OAuth failure on ${path}:`, {
            error: locUrl.searchParams.get("error"),
            error_description: locUrl.searchParams.get("error_description"),
            location,
          });
        } catch {
          console.error(
      ...
      if (
        ...
        try {
          ...
          // (session_token plus state/code_verifier deletions), so we need
          // getSetCookie() to retrieve them all.
 ...
 * Diagnostic endpoint that prints the exact redirect URIs Better Auth expects
 * for each OAuth provider. Use this to verify Google Cloud Console / GitHub
 * OAuth app settings. Safe to expose — credentials are never returned.
 */
auth.get("/oauth-config", async (c) => {
  ...
  return c.json({
    ...
    appUrl: env.APP_URL,
    redirectUris: {
      google: `${baseUrl}/callback/google`,
      github: `${baseUrl}/callback/github`,
      discord: `${baseUrl}/callback/discord`,
      twitter: `${baseUrl}/callback/twitter`,
    },
    providers: {
    ...
    emailBinding: typeof env.EMAIL?.send === "function",
    crossSubdomainCookieDomain: env.APP_URL?.includes("localhost")
      ? null
      : `.${new URL(env.APP_URL).hostname.split(".").slice(-2).join(".")}`,
    note: "If Google OAuth fails, the redirectUris.google value MUST be listed under 'Authorized redirect URIs' in Google Cloud Console for the OAuth client.",
  });
});
