...
aiRouter.post("/preview", zValidator("json", previewSchema), async (c) => {
  ...
  try {
    const preview = await generateAILinkPreview(c.env, url); // Removed pageContent as it fetches it
    return ok(c, preview);
  } catch (error) {
...
aiRouter.get("/scheduling", async (c) => {
  ...
  if (analysis.confidence > 0.4) {
    try {
      ...
      const recommendation = await generateSchedulingRecommendation(c.env, topSlots);
