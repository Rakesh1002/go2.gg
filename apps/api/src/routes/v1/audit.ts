...
// All audit routes require auth — without this every handler short-
// circuited to 403 "Organization required" because c.get("user") was empty.
audit.use("/*", apiKeyAuthMiddleware());
...
audit.get("/logs", zValidator("query", auditLogsQuerySchema), async (c) => {
  ...
  const repos = createD1Repositories(drizzle(c.env.DB, { schema }));
  ...
  const membership = await repos.members.findByUserAndOrg(user.id, user.organizationId);
