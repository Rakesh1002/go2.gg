...
const importQuerySchema = z.object({
  ...
  /** Optional folder UUID to drop every imported link into. */
...
bulk.post("/import", zValidator("query", importQuerySchema), async (c) => {
  ...
  const { format, dryRun, folderId } = c.req.valid("query");
  const db = drizzle(c.env.DB, { schema });

  // Validate target folder up-front so we don't import 500 links and then
  // discover the folder doesn't exist.
  if (folderId) {
    const target = await db
      ...
      .limit(1);
    if (!target[0]) return notFound(c, "Folder not found");
    const access = checkFolderAccess(target[0], user, "write");
  ...
  // If links landed in a folder, refresh its cached link count.
  if (folderId && createdLinks.length > 0) {
