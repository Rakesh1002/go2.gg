...
slugs.post("/suggest", zValidator("json", suggestSchema), async (c) => {
  ...
  try {
    ...
    const aiSuggestions = await generateAISlugs(c.env, input.url, {
      count: input.count,
      style: input.style,
...
slugs.post("/preview", zValidator("json", previewSchema), async (c) => {
  ...
  try {
    const preview = await generateAILinkPreview(c.env, input.url);
...
slugs.post("/scheduling", async (c) => {
  try {
    ...
    const recommendations = await generateSchedulingRecommendation(c.env, body.clickData);
