...
stats.get("/dashboard", async (c) => {
  ...
  try {
    await ensureUserHasOrganization(db, user.id, user.email, user.name ?? undefined, c.env);
  } catch (error) {
...
stats.get("/links", async (c) => {
  ...
  // Use the clicks.userId column directly — simpler and more robust than
  // building a 38-id IN-list at request time.
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  const todayIso = today.toISOString();
  ...
  const clicksTodayResult = await db
    ...
    .where(
      and(
        ...
        sql`${schema.clicks.timestamp} >= ${todayIso}`,
      ),
    );

  const clicksToday = Number(clicksTodayResult[0]?.count) || 0;
  ...
  const topCountryResult = await db
    ...
    .where(
      and(
        eq(schema.clicks.userId, user.id),
        sql`${schema.clicks.timestamp} >= ${thirtyDaysAgoIso}`,
        sql`${schema.clicks.country} IS NOT NULL`,
      ),
    )
