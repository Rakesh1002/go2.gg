...
import { logError } from "../lib/axiom.js";
...
export const errorHandler: ErrorHandler<{ Bindings: Env }> = (err, c) => {
  ...
  // Tunnel to Axiom (no-op when AXIOM_API_KEY is unset).
  // Skip AuthError 4xx — those are user errors, not infrastructure noise.
  const isUserError = err instanceof AuthError && err.status < 500;
  if (!isUserError) {
    c.executionCtx?.waitUntil(
      logError(c.env, err.message || "Unhandled API error", {
        requestId,
        name: err.name,
        path: c.req.path,
        method: c.req.method,
