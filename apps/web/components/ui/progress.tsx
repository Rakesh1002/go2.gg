  ...
  "h-full w-full flex-1 rounded-full transition-transform duration-500 ease-out",
  {
    variants: {
      variant: {
        ...
        gradient:
          "animate-shimmer bg-[length:200%_100%] bg-gradient-to-r from-primary via-secondary to-primary",
      },
    },
    defaultVariants: {
      variant: "default",
    ...
    <div className={cn("relative inline-flex", className)} style={{ width: size, height: size }}>
      ...
      {showValue && (
        <div className="absolute inset-0 flex items-center justify-center">
          <span className="font-semibold text-xs tabular-nums">{Math.round(displayValue)}%</span>
        </div>
      )}
    </div>
  );
