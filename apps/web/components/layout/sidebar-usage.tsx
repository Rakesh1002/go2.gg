    ...
    <div className={cn("border-t px-3 py-3", className)}>
      ...
      <div className="mb-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-muted-foreground text-xs capitalize">{plan} Plan</span>
        </div>
        {expanded ? (
          <button
            ...
            <div key={i} className="space-y-1">
              <div className="flex items-center justify-between text-xs">
                ...
                <span className={cn("font-medium tabular-nums", isNearLimit && "text-amber-500")}>
                  ...
                  {isUnlimited && <span className="ml-0.5 text-muted-foreground">used</span>}
                </span>
              </div>
              {!isUnlimited && (
                <Progress
      ...
      {items.length > 2 && !expanded && (
        <button
          ...
          className="mt-2 w-full text-muted-foreground text-xs transition-colors hover:text-foreground"
        >
          Show more
        </button>
      )}
      ...
      {/* Upgrade button */}
      {(plan === "free" || isTrial) && (
        <Button asChild size="sm" className="mt-3 w-full gap-1.5" variant="outline">
