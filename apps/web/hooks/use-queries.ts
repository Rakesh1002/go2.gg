...
export const queryKeys = {
  ...
  folderAnalytics: (id: string, period: string) => ["folders", id, "analytics", period] as const,
...
export interface LinkQueryParams {
  ...
  search?: string;
  sort?: string;
  /** UUID = scope to that folder, "none" = links without a folder */
...
/**
 * Hook to fetch the caller's folders. Returns empty list on error so the
 * picker can render a graceful fallback instead of throwing.
 */
async function fetchFolders(): Promise<Folder[]> {
  const response = await fetch(`${API_URL}/api/v1/folders?perPage=100`, {
    credentials: "include",
  });
...
async function fetchFolderAnalytics(id: string, period: string): Promise<FolderAnalytics> {
  const response = await fetch(`${API_URL}/api/v1/analytics/folders/${id}?period=${period}`, {
    credentials: "include",
  });
  if (!response.ok) throw new Error("Failed to fetch folder analytics");
  const result = await response.json();
  return result.data;
}

export function useFolderAnalytics(id: string, period: "7d" | "30d" | "90d" = "30d") {
  return useQuery({
    queryKey: queryKeys.folderAnalytics(id, period),
    queryFn: () => fetchFolderAnalytics(id, period),
    enabled: !!id,
    staleTime: 30_000,
  });
}
