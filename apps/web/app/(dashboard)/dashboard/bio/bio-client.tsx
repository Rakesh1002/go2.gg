...
const ITEM_TYPES = [
  { id: "link", name: "Link", icon: Link2, description: "A clickable link card" },
  { id: "email_signup", name: "Email signup", icon: Mail, description: "Collect email subscribers" },
  { id: "embed", name: "Embed", icon: Youtube, description: "YouTube or Spotify embed" },
  { id: "image", name: "Image", icon: Image, description: "A static image" },
  { id: "header", name: "Header", icon: Type, description: "Section header text" },
  { id: "divider", name: "Divider", icon: Minus, description: "Visual separator" },
] as const;
...
export function BioClient() {
  ...
  async function toggleHideBranding(id: string, hideBranding: boolean) {
    try {
      const response = await fetch(`${API_URL}/api/v1/galleries/${id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
      ...
      if (response.ok) {
        setGalleries((prev) => prev.map((g) => (g.id === id ? { ...g, hideBranding } : g)));
        if (selectedGallery?.id === id) {
          setSelectedGallery((prev) => (prev ? { ...prev, hideBranding } : null));
        }
        toast.success(hideBranding ? "Branding hidden" : "Branding shown");
      } else {
        throw new Error("Failed to update gallery");
      }
    } catch (error) {
            ...
            <TabsContent value="settings" className="space-y-4">
              <Card>
                ...
                <CardContent className="space-y-6">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <Label>Hide &ldquo;Powered by Go2&rdquo;</Label>
                      <p className="text-muted-foreground text-sm">
                        Remove the Go2 footer credit from this bio page.
                      </p>
                    </div>
                    <Switch
                      checked={!!selectedGallery.hideBranding}
                      onCheckedChange={(checked) =>
                        toggleHideBranding(selectedGallery.id, checked)
                      }
                    />
                  </div>
                ...
                <CardContent>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="rounded-lg border p-4">
                      ...
                      <p className="font-bold text-3xl">{selectedGallery.viewCount}</p>
                    </div>
                    <div className="rounded-lg border p-4">
                ...
                <div className="mx-auto max-w-[320px] rounded-3xl border-4 border-muted bg-background p-4 shadow-lg">
                  <div className="flex flex-col items-center text-center">
                    {selectedGallery.avatarUrl ? (
                      ...
                      <div className="flex h-20 w-20 items-center justify-center rounded-full bg-primary/10 font-bold text-2xl text-primary">
                    ...
                    {selectedGallery.bio && (
                      <p className="mt-2 text-muted-foreground text-xs">{selectedGallery.bio}</p>
                    )}
                        ...
                        .map((item) => (
                          <div key={item.id}>
                            {item.type === "divider" ? (
                              ...
                              <div className="cursor-pointer rounded-lg border bg-card p-3 font-medium text-sm transition-colors hover:bg-muted">
                                {item.title}
                              </div>
                            )}
                          </div>
