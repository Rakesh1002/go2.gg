...
export function TeamManagement() {
  ...
  const fetchMembers = async (orgId: string) => {
    try {
      // Resolve the current user from the session at the same time as the
      // members list. localStorage.getItem("userId") was the previous lookup
      // — but nothing in the app actually writes that key, so the role
      // resolved to null, canManageMembers stayed false, and the Invite
      // Member card never rendered.
      const [membersRes, meRes] = await Promise.all([
        fetch(`${API_URL}/api/v1/organizations/${orgId}/members`, {
          credentials: "include",
        }),
      ...
      const me = meJson?.data ?? null;

      const list: Member[] = membersJson.data || [];
      setMembers(list);

      const myMembership = list.find(
        (m) =>
          (me?.id && m.userId === me.id) ||
          (me?.email && m.user?.email?.toLowerCase() === me.email.toLowerCase())
      );
  ...
  const handleInvite = async () => {
    ...
    if (!currentOrgId) {
      toast.error("Workspace isn't loaded yet — try again in a second");
      return;
    }
