    ...
    <div className="flex flex-wrap justify-center gap-8 p-8">
      {[
        ...
        <div key={i} className="text-center">
          <div
            className="rounded-xl border border-[var(--marketing-border)] bg-[var(--marketing-bg-elevated)] p-4 shadow-lg"
          ...
          <p className="mt-2 text-[var(--marketing-text-muted)] text-sm">{colors.label}</p>
        </div>
      ))}
    </div>
  );
...
export default function QRCodesPage() {
  return (
    <FeaturePageTemplate
      ...
      agentCallout={{
        title: "Agents can mint branded QR codes for the links they create.",
        body:
          "POST /api/v1/qr accepts a link id plus styling — colors, logo, dot style, error-correction. An agent generating a campaign link can attach a matching QR in the same call, scoped to your workspace and your brand. Click attribution and scan attribution land in the same dashboard.",
        primitive: "POST /api/v1/qr · qrCodes table · qr.scanned webhook",
      }}
