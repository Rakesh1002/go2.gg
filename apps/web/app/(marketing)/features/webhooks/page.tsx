...
export default function WebhooksFeaturePage() {
  return (
    <FeaturePageTemplate
      ...
      agentCallout={{
        title: "Webhooks are the easiest way to wire agents into your trace UI.",
        body:
          "Subscribe to click, link.created, link.updated, link.deleted, qr.scanned, and domain.verified. Every payload includes the agent context (agent_id, agent_run_id, agent_actor_id) so your trace UI, Slack alerts, or run replays can correlate a click back to the agent run that minted the link. HMAC-signed, retried with backoff.",
        primitive: "POST /api/v1/webhooks · webhook_deliveries · HMAC signing",
      }}
    />
  );
}
